...
class CreatePromotionViewModel : ViewModel() {
    ...
    private fun validate() {
        ...
        val limTotalOk = limTotal >= 0
        val limUserOk  = limUser  >= 0
        // Regla: si ambos > 0, per-user no puede exceder total
        val limRelOk = !((limTotal > 0) && (limUser > 0) && (limUser > limTotal))

        val valid = idOk && titleOk && descOk && datesOk && pctOk && priceOk && limTotalOk && limUserOk && limRelOk
    ...
    private fun submit() {
        ...
        // Prefijo de mecánica en descripción para tipos no % ni precio fijo
